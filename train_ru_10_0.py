...
from utils.data_utils import load_ru, load_ru_one, fix_seed, count_parameters
...
if __name__ == '__main__':
    ...
    train_set = test_set = load_ru_one(
        args.data_path, 'train', condition_config['data_type']
    )
    ...
    if args.model_ckpt != '':
        print('[INFO] loading model from', args.model_ckpt)
        weight = torch.load(os.path.join(args.model_ckpt, 'model.pth'), map_location=device)
        weight = {
            k.replace('base', 'solvent') : v for k, v in weight.items()
        }
