...
from utils.training import train_mol_yield_freeze, eval_mol_yield
from utils.Dataset import cn_colfn

from model import (
    CNYieldModel, RAlignEncoderExtend,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--n_adaptor', type=int, default=0,
        help='Number of layers with condition adaptor' +
        'Non-Positive value: all layers are equipped with adatpor' +
        'Positive value: specifies how many top layers' +
        ' (counting backwards from the last layer) are ' +
        'equipped with the adaptor.'
    )
    ...
    if args.n_adaptor >= 0:
        nada = min(args.n_layer, args.n_adaptor)
        condition_infos = [{}] * (args.n_layer - nada) + \
            [condition_infos] * nada

    encoder = RAlignEncoderExtend(
