...
def validate(log_dir):
    ...
    with open(log_json, 'r') as f:
        log = json.load(f)
    if 'args' not in log or 'valid_metric' not in log  or 'train_loss' not in log:
        return None

    if len(set([log['args']['epoch'], len(log['train_loss']), len(log['valid_metric'])])) != 1:
        return None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iid_marker', '-iid', type=str, default='final', help='Marker for IID datasets')
    ...
    for root, dirs, files in os.walk(input_file):
        ...
        best_test_result = log['valid_metric'][best_ep]
