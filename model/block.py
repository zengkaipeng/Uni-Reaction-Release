...
class DualGATEncoder(torch.nn.Module):
    def __init__(
        ...
        for i in range(n_layer):
            update_edge = (i < n_layer - 1) or update_last_edge
            self.layers.append(DualGATBlock(
                emb_dim=emb_dim, heads=heads, edge_dim=edge_dim,
                reac_batch_infos=reac_batch_infos, reac_num_keys=reac_num_keys,
                prod_batch_infos=prod_batch_infos, prod_num_keys=prod_num_keys,
                negative_slope=negative_slope, dropout=dropout,
...
class RAlignEncoderExtend(torch.nn.Module):
    def __init__(
        self, n_layer, emb_dim, heads, edge_dim, reac_batch_infos={},
        reac_num_keys={}, prod_batch_infos={}, prod_num_keys={},
        dropout=0.1, negative_slope=0.2, update_last_edge=False
    ):
        super(RAlignEncoderExtend, self).__init__()
        self.n_layers = n_layer
        self.layers = torch.nn.ModuleList()
        for i in range(n_layer):
            update_edge = (i < n_layer - 1) or update_last_edge
            if isinstance(reac_num_keys, list):
                this_reac_num = reac_num_keys[i]
            else:
                this_reac_num = reac_num_keys
            if isinstance(prod_num_keys, list):
                this_prod_num = prod_num_keys[i]
            else:
                this_prod_num = prod_num_keys

            if isinstance(reac_batch_infos, list):
                this_reac_batch = reac_batch_infos[i]
            else:
                this_reac_batch = reac_batch_infos

            if isinstance(prod_batch_infos, list):
                this_prod_batch = prod_batch_infos[i]
            else:
                this_prod_batch = prod_batch_infos

            self.layers.append(RAlignGATBlock(
                emb_dim=emb_dim, heads=heads, edge_dim=edge_dim,
                reac_batch_infos=this_reac_batch, reac_num_keys=this_reac_num,
                prod_batch_infos=this_prod_batch, prod_num_keys=this_prod_num,
                negative_slope=negative_slope, dropout=dropout,
                edge_update=update_edge
            ))
